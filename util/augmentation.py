...
class BasicTransforms:
    ...
    def __call__(self, image): #이미지에 트랜스폼 적용함
        if self.augment:
...
class AlbumentationsTransforms:
    def __init__(self, augment=False): #True면 증강을 포함한 트랜스폼 적용, False면 기본 트랜스폼 적용
        self.augment = augment
    ...
    def __call__(self, image): #이미지에 트랜스폼 적용
        if self.augment:
...
class CutMixTransforms:
    def __init__(self, alpha=1.0): #알파: CutMix에서 사용할 베타 분포 파라미터
        self.alpha = alpha

    def __call__(self, image1, image2, label1, label2): #두 개 이미지를 컷믹스
    ...
    @staticmethod
    def rand_bbox(size, lam): #람다 값을 기준으로 랜덤한 바운딩 박스 생성
        W = size[2]
        H = size[1]
...
class MixUpTransforms:
    def __init__(self, alpha=1.0): #알파: 믹스업에서 사용할 베타 분포의 파라미터
        self.alpha = alpha

    def __call__(self, image1, image2, label1, label2): #두 개 이미지를 믹스업
        lam = random.beta(self.alpha, self.alpha)  # 베타 분포로 람다 값 생성
...
class TransformSelector: #사용자가 지정한 transform_type에 따라 서로 다른 변환 객체를 반환하는 구조
    ...
    def get_transform(self, augment=False, alpha=1.0):
        """
        augment: 데이터 증강 여부
